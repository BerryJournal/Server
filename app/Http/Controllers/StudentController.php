...
class StudentController extends Controller
{
    ...
    function getSubject(Request $request, $id) {
        ...
        return response()->json(['message' => ['date' => Date::with(['marks' => function($query) use ($request) {
        ...
        'skips_count' => Skip::where('student_id', $request->user()->id)
            ...
            })->count(), 'average_mark' => Mark::where('student_id', $request->user()->id)
            ->whereHas('date', function($query) use ($id) {
                $query->where('group_subject_id', $id);
            })->whereNotIn('mark', ['нз', 'зч'])->avg('mark'), 'final_mark' => Final_Mark::where('group_subject_id', $id)->where('student_id', $request->user()->id)->first('mark')]]);
    }
