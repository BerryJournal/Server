...
class TeacherController extends Controller
{
    ...
    function getMyGroups(Request $request) {
        return response()->json(['message' => Group::with([
    ...
    $groupedDates[$monthRu]['days'][] = $day;
        ...
        return ['message' => ['subject'=> Group_Subject::with('group', 'subject')->find($id),
        'students' => User::where('group_id', Group_Subject::find($id)->group_id)->where('isArchive', false)->orderBy('surname')->orderBy('name')->get(['id', 'name', 'surname', 'patronymic']),
         ...
         'average_marks' => Mark::selectRaw('student_id, AVG(mark) as average_mark')->whereNotIn('mark', ['НЗ', 'ЗЧ'])->groupBy('student_id')->get()->map(function ($item) {return ['student_id' => $item->student_id, 'average_mark' => (float)$item->average_mark];})->toArray()]];
    }
